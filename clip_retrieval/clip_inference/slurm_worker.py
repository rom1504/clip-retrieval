...
def get_task_list(num_tasks, world_size, global_rank):
    ...
    print(f"worker {global_rank} processing tasks {tasks}")
...
def slurm_worker():
    ...
    tasks = get_task_list(num_tasks, world_size, global_rank)

    # Process the tasks
    worker(tasks=tasks, **worker_args)
