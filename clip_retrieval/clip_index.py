...
def clip_index(embeddings_folder, index_folder, max_index_memory_usage="4G"):
    quantizer = Quantizer()
    if os.path.exists(embeddings_folder+"/img_emb"):
        quantizer.quantize(embeddings_path=embeddings_folder+"/img_emb", output_path=index_folder+"/img_index", max_index_memory_usage=max_index_memory_usage, current_memory_available="16G")
    ...
    if os.path.exists(embeddings_folder+"/text_emb"):
        quantizer.quantize(embeddings_path=embeddings_folder+"/text_emb", output_path=index_folder+"/txt_index", max_index_memory_usage=max_index_memory_usage, current_memory_available="16G")
